    ...
    Type, extends(TInitialPower) :: TInitialPowerLaw
        ...
        ! New parameters in the fortran code 
        real(dl) :: amp = 0.0599_dl
        real(dl) :: freq = 280._dl
        real(dl) :: wid = 0.08_dl
        real(dl) :: centre = 0.2_dl
        real(dl) :: phase = 0.0_dl
    ...
    feature = (1+ sin(this%phase+k*this%freq)*this%amp*exp(-(k-this%centre)* (k-this%centre)/(this%wid*this%wid)))
    TInitialPowerLaw_ScalarPower = this%As * exp(lnrat * (this%ns - 1 + &
        &             lnrat * (this%nrun / 2 + this%nrunrun / 6 * lnrat))) * feature
